...
class RNNLM_Model(LanguageModel):
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ...
            inputs = tf.unstack(embed, axis=1)
            # END YOUR CODE
            return inputs
    ...
    def add_projection(self, rnn_outputs):
        ...
        """
        # YOUR CODE HERE
        with tf.variable_scope('projection', initializer=
                               tf.contrib.layers.xavier_initializer()):
            U = tf.get_variable(
                'U', shape=(self.config.hidden_size, len(self.vocab)))
    ...
    def add_loss_op(self, output):
        ...
        """
        # YOUR CODE HERE
        # Same loss applied across time steps
        weights = [tf.ones((self.config.batch_size * self.config.num_steps,))]
        labels = [tf.reshape(self.labels_placeholder, (-1,))]
        # using tf api v1 - using older version of sequence_loss which takes in
        loss = tf.contrib.legacy_seq2seq.sequence_loss([output], labels, weights)
        # END YOUR CODE
        return loss
    ...
    def __init__(self, config):
        ...
        # SB: updated tf.concat to match v1.0 api
    ...
    def add_model(self, inputs):
        ...
        with tf.variable_scope('RNN', initializer=tf.contrib.layers.xavier_initializer()):
            ...
            cell = lambda h, x: tf.sigmoid(tf.matmul(h, H) + tf.matmul(x, I) + b_1)
            #cell = lambda h, x: tf.sigmoid(tf.matmul(h, H) + tf.matmul(x, I) + b_1)

            rnn_outputs = [cell(self.initial_state, inputs[0])]
            state = rnn_outputs[0]
            tf.get_variable_scope().reuse_variables()
            for x in inputs[1:]:
                H = tf.get_variable(
